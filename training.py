...
class Mysql:
    ...
    def query_calibration_value(self):
        ...
        value_list = []
        for v in q: value_list.append(v[0])
        return value_list
    ...
    def query_calibration_by_value(self, value):
        ...
        timestamp_list = []
        for t in q: timestamp_list.append(t[0])
        return timestamp_list

    def query_calibration_by_value_timestamp(self, value, timestamp):
        q = self.session.query(CalibrationOriginal.bssid, CalibrationOriginal.signal_strength).filter(CalibrationOriginal.calibration_value == value, CalibrationOriginal.calibration_timestamp_id == timestamp, CalibrationOriginal.signal_strength <= -50, CalibrationOriginal.signal_strength >= -90)
        ap_list = []
        for ap in q:
...
if __name__ == "__main__":
    ...
    for v in value_list:
        ...
        for t in timestamp_list:
            #print "T : %d" % t
            ap_no = ap_no + 1
            onetime_ap_list = training_db.query_calibration_by_value_timestamp(v, t)
            all_ap_list.extend(onetime_ap_list)
        #print "All times get AP : %s" % all_ap_list
        #print "N : %d" % ap_no
        training_ap_list = []
        for ap in all_ap_list:
            if all_ap_list.count(ap) >= ap_no * 0.7:
                if training_ap_list.count(ap) == 0:
                    training_ap_list.append(ap)
        #print "Training AP list : %s" % training_ap_list
        for ap in training_ap_list:
            one_ap_value_list = training_db.query_calibration_by_value_bssid(v, ap)
            #print "AP bssid : %s" % ap
            #print "Training ap value list : %s" % one_ap_value_list
            ap_training_sum_value = 0.0
            n = 0
            for i in one_ap_value_list:
                ap_training_sum_value = ap_training_sum_value + (0.1 ** i)
                n = n + 1
            ap_training_value = ap_training_sum_value / n
