...
class CalibrationHandler(BaseHandler):
    def post(self):
        ...
        if cp["calibration_value"] == -1:
            #print cp["bssid"], cp["timestamp"]
            if cp["timestamp"] > timeid:
                #if CP == [] and len(CP) < 4: return
                #print CP
                clb.realCalibration(CP)
                timeid = cp["timestamp"]
                CP = []
                CP.append([cp["calibration_value"], str(cp["bssid"]), cp["signal_strength"]])
            else:
                CP.append([cp["calibration_value"], str(cp["bssid"]), cp["signal_strength"]])
